...
* 本仓库发布的CollectWool项目中涉及的任何解锁和解密分析脚本，仅用于测试和学习研究，不能保证其合法性，准确性，完整性和有效性，请根据情况自行判断.

* CollectWoolTeam对任何脚本问题概不负责，包括但不限于由任何脚本错误导致的任何损失或损害.

* 间接使用脚本的任何用户，包括但不限于建立VPS或在某些行为违反国家/地区法律或相关法规的情况下进行传播, CollectWoolTeam 对于由此引起的任何隐私泄漏或其他后果概不负责.

* 请勿将CollectWool项目的任何内容用于商业或非法目的，否则后果自负.
